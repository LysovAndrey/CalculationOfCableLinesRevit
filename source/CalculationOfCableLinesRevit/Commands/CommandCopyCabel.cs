...
namespace CalculationCable {
  ...
  internal class CommandCopyCabel : IExternalCommand {
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements) {
      ...
      if (copyElement.Count != 0) {

        var doc = cables.ActiveDocument;

        using (Transaction t = new Transaction(cables.ActiveDocument)) {
          t.Start("Копирование элементов");

          foreach (var item in copyElement) {
            var cabel = doc.GetElement(ElementTransformUtils.CopyElement(doc, item.Element.Id, new XYZ(0, 0, 0)).First());
          ...
          doc.Delete(copyElement[0].Element.Id);
